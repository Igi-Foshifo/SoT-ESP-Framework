...
from data.mapping import merchantcrates
from modules.display_object import DisplayObject

MERCH_LOOT_COLOR = (0, 0, 255)
MERCH_LOOT_LABEL_COLOR = (0, 0, 255, 255)
CIRCLE_SIZE = 3  # The size of the indicator circle we want


class MerchLoot(DisplayObject):
    """
    Class to generate information for a merchant loot object in memory
    """
    ...
    def __init__(self, memory_reader, actor_id, address, my_coords, raw_name):
        ...
        All of this data represents a "Merchant Loot Item". If you want to add more, you will
        ...
        # Generate our Merchant Loot's info
        self.name = merchantcrates.get(self.raw_name).get("Name")
    ...
    def _build_text_render(self) -> Label:
        ...
        :rtype: Label
        :return: What text we want displayed next to the merchant loot
        """
        if self.screen_coords:
            return Label(self.text_str,
                         color=MERCH_LOOT_LABEL_COLOR,
        ...
        return Label(self.text_str, color=MERCH_LOOT_LABEL_COLOR,
                     font_name=TEXT_FONT_NAME, font_size=TEXT_FONT_SIZE, x=0, y=0, dpi=TEXT_DPI, batch=main_batch)

    def update(self, my_coords: dict):
        """
        A generic method to update all the interesting data about a merchant loot
