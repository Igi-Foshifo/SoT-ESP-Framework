...
class Crews(DisplayObject):
    ...
    def _get_crews_info(self):
        ...
        for x in range(0, crews[1]):
            ...
            # get the crews MaxMatchmakingPlayers int value to determine the crew's ship type
            crew_session_raw = self.rm.read_int(crews[0] +
                                                (OFFSETS.get('Crew.Size') * x) +
                                                OFFSETS.get('Crew.CrewSessionTemplate') +
                                                OFFSETS.get('CrewSessionTemplate.MaxMatchmakingPlayers'))

            crew_ship = ""
            if crew_session_raw == 2:
                crew_ship = "Sloop"
            elif crew_session_raw == 3:
                crew_ship = "Brig"
            elif crew_session_raw == 4:
                crew_ship = "Galleon"
