...
from utils.helpers import calculate_distance, object_to_screen, main_batch, TEXT_OFFSET_X, TEXT_OFFSET_Y
from modules.display_object import DisplayObject


class Debug(DisplayObject):
    """
    Class to generate information for a actor object in memory
    """
    ...
    def __init__(self, memory_reader, actor_id, address, my_coords, raw_name):
        ...
        All of this data represents an "actor". If you want to add more, you will
    ...
    def _build_text_render(self) -> Label:
        ...
        :rtype: Label
        :return: What text we want displayed next to the actor
        """
        if self.screen_coords:
        ...
        return Label(self.text_str, color=(0, 0, 0, 0), x=0, y=0, batch=main_batch)

    def update(self, my_coords: dict):
        """
        A generic method to update all the interesting data about an actor
        ...
        if self.screen_coords:
            # render actors when under 500m
            if "BP_" in self.name and new_distance < 500:
