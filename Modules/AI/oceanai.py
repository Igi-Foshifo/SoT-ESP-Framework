...
from Modules.mapping import oceanai
from Modules.display_object import DisplayObject

OCEANAI_COLOR = (255, 255, 255)
OCEANAI_LABEL_COLOR = (150, 0, 0, 255)
CIRCLE_SIZE = 2  # The size of the indicator circle we want


class OceanAI(DisplayObject):
    """
    Class to generate information for an ocean enemy object in memory
    """
    ...
    def __init__(self, memory_reader, actor_id, address, my_coords, raw_name):
        ...
        All of this data represents an "ocean". If you want to add more, you will
        ...
        # Generate our ocean enemy's info
        self.name = oceanai.get(self.raw_name).get("Name")
    ...
    def _build_text_render(self) -> Label:
        ...
        :rtype: Label
        :return: What text we want displayed next to the ocean enemy
        """
        if self.screen_coords:
            return Label(self.text_str,
                         color=OCEANAI_LABEL_COLOR,
        ...
        return Label(self.text_str, color=OCEANAI_LABEL_COLOR, x=0, y=0, batch=main_batch)

    def update(self, my_coords: dict):
        """
        A generic method to update all the interesting data about an ocean enemy
