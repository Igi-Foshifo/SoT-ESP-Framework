...
from Modules.mapping import landai
from Modules.display_object import DisplayObject

LANDAI_COLOR = (255, 255, 255)
LANDAI_LABEL_COLOR = (150, 0, 0, 255)
CIRCLE_SIZE = 2  # The size of the indicator circle we want


class LandAI(DisplayObject):
    """
    Class to generate information for a land enemy object in memory
    """
    ...
    def __init__(self, memory_reader, actor_id, address, my_coords, raw_name):
        ...
        All of this data represents a "land enemy". If you want to add more, you will
        ...
        # Generate our land enemy's info
        self.name = landai.get(self.raw_name).get("Name")
    ...
    def _build_text_render(self) -> Label:
        ...
        :rtype: Label
        :return: What text we want displayed next to the land enemy
        """
        if self.screen_coords:
            return Label(self.text_str,
                         color=LANDAI_LABEL_COLOR,
        ...
        return Label(self.text_str, color=LANDAI_LABEL_COLOR, x=0, y=0, batch=main_batch)

    def update(self, my_coords: dict):
        """
        A generic method to update all the interesting data about a land enemy
