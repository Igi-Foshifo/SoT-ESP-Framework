...
ASHEN_LORD_LABEL_COLOR = (150, 0, 0, 255)
CIRCLE_SIZE = 2  # The size of the indicator circle we want


class AshenLord(DisplayObject):
    """
    Class to generate information for an ashen lord object in memory
    """
    ...
    def __init__(self, memory_reader, actor_id, address, my_coords, raw_name):
        ...
        All of this data represents an "ashen lord". If you want to add more, you will
        ...
        # Generate our ashen lord's info
        self.name = ashlord.get(self.raw_name).get("Name")
    ...
    def _build_text_render(self) -> Label:
        ...
        :rtype: Label
        :return: What text we want displayed next to the ashen lord
        """
        if self.screen_coords:
            return Label(self.text_str,
                         color=ASHEN_LORD_LABEL_COLOR,
        ...
        return Label(self.text_str, color=ASHEN_LORD_LABEL_COLOR, x=0, y=0, batch=main_batch)

    def update(self, my_coords: dict):
        """
        A generic method to update all the interesting data about an ashen lord
