...
from Modules.mapping import rowboats
from Modules.display_object import DisplayObject

ROWBOAT_COLOR = (255, 255, 255)
ROWBOAT_LABEL_COLOR = (255, 255, 255, 255)
CIRCLE_SIZE = 2  # The size of the indicator circle we want


class Rowboat(DisplayObject):
    """
    Class to generate information for a rowboat object in memory
    """
    ...
    def __init__(self, memory_reader, actor_id, address, my_coords, raw_name):
        ...
        All of this data represents a "Rowboat". If you want to add more, you will
        ...
        # Generate our rowboat's info
        self.name = rowboats.get(self.raw_name).get("Name")
    ...
    def _build_text_render(self) -> Label:
        ...
        :rtype: Label
        :return: What text we want displayed next to the rowboat
        """
        if self.screen_coords:
            return Label(self.text_str,
                         color=ROWBOAT_LABEL_COLOR,
                         x=self.screen_coords[0] + TEXT_OFFSET_X,
                         y=self.screen_coords[1] + TEXT_OFFSET_Y,
                         batch=main_batch)

        return Label(self.text_str, color=ROWBOAT_LABEL_COLOR, x=0, y=0, batch=main_batch)

    def update(self, my_coords: dict):
        """
        A generic method to update all the interesting data about a rowboat
