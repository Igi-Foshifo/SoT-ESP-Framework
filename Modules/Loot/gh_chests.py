...
from Modules.mapping import ghchests
from Modules.display_object import DisplayObject

GH_CHEST_COLOR = (204, 204, 0)
GH_CHEST_LABEL_COLOR = (204, 204, 0, 255)
CIRCLE_SIZE = 3  # The size of the indicator circle we want


class GHChest(DisplayObject):
    """
    Class to generate information for a gold hunter chest object in memory
    """
    ...
    def __init__(self, memory_reader, actor_id, address, my_coords, raw_name):
        ...
        All of this data represents a "Gold Hunter Chest". If you want to add more, you will
        ...
        # Generate our gold hunter chest's info
        self.name = ghchests.get(self.raw_name).get("Name")
    ...
    def _build_text_render(self) -> Label:
        ...
        :rtype: Label
        :return: What text we want displayed next to the gold hunter chest
        """
        if self.screen_coords:
            return Label(self.text_str,
                         color=GH_CHEST_LABEL_COLOR,
                         x=self.screen_coords[0] + TEXT_OFFSET_X,
                         y=self.screen_coords[1] + TEXT_OFFSET_Y,
                         batch=main_batch)

        return Label(self.text_str, color=GH_CHEST_LABEL_COLOR, x=0, y=0, batch=main_batch)

    def update(self, my_coords: dict):
        """
        A generic method to update all the interesting data about a gold hunter chest
