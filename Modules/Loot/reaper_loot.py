...
from Modules.mapping import reaploot
from Modules.display_object import DisplayObject

REAPER_LOOT_COLOR = (255, 255, 255)
REAPER_LOOT_LABEL_COLOR = (255, 255, 255, 255)
CIRCLE_SIZE = 2  # The size of the indicator circle we want


class ReaperLoot(DisplayObject):
    """
    Class to generate information for a reaper loot object in memory
    """
    ...
    def __init__(self, memory_reader, actor_id, address, my_coords, raw_name):
        ...
        All of this data represents a "Reaper Loot". If you want to add more, you will
        ...
        # Generate our reaper loot's info
        self.name = reaploot.get(self.raw_name).get("Name")
    ...
    def _build_text_render(self) -> Label:
        ...
        :rtype: Label
        :return: What text we want displayed next to the reaper loot
        """
        if self.screen_coords:
            return Label(self.text_str,
                         color=REAPER_LOOT_LABEL_COLOR,
        ...
        return Label(self.text_str, color=REAPER_LOOT_LABEL_COLOR, x=0, y=0, batch=main_batch)

    def update(self, my_coords: dict):
        """
        A generic method to update all the interesting data about a reaper loot
