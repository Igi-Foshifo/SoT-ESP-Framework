...
from Modules.mapping import trident
from Modules.display_object import DisplayObject

TRIDENT_COLOR = (255, 255, 255)
TRIDENT_LABEL_COLOR = (255, 255, 255, 255)
CIRCLE_SIZE = 2  # The size of the indicator circle we want


class Trident(DisplayObject):
    """
    Class to generate information for a trident object in memory
    """
    ...
    def __init__(self, memory_reader, actor_id, address, my_coords, raw_name):
        ...
        All of this data represents a "Trident". If you want to add more, you will
        ...
        # Generate our trident's info
        self.name = trident.get(self.raw_name).get("Name")
    ...
    def _build_text_render(self) -> Label:
        ...
        :rtype: Label
        :return: What text we want displayed next to the trident
        """
        if self.screen_coords:
            return Label(self.text_str,
                         color=TRIDENT_LABEL_COLOR,
        ...
        return Label(self.text_str, color=TRIDENT_LABEL_COLOR, x=0, y=0, batch=main_batch)

    def update(self, my_coords: dict):
        """
        A generic method to update all the interesting data about a trident
