...
from Modules.mapping import medallions
from Modules.display_object import DisplayObject

MEDALLION_COLOR = (255, 255, 255)
MEDALLION_LABEL_COLOR = (255, 255, 255, 255)
CIRCLE_SIZE = 2  # The size of the indicator circle we want


class Medallion(DisplayObject):
    """
    Class to generate information for a medallion object in memory
    """
    ...
    def __init__(self, memory_reader, actor_id, address, my_coords, raw_name):
        ...
        All of this data represents a "Medallion". If you want to add more, you will
        ...
        # Generate our medallion's info
        self.name = medallions.get(self.raw_name).get("Name")
    ...
    def _build_text_render(self) -> Label:
        ...
        :rtype: Label
        :return: What text we want displayed next to the medallion
        """
        if self.screen_coords:
            return Label(self.text_str,
                         color=MEDALLION_LABEL_COLOR,
        ...
        return Label(self.text_str, color=MEDALLION_LABEL_COLOR, x=0, y=0, batch=main_batch)

    def update(self, my_coords: dict):
        """
        A generic method to update all the interesting data about a medallion
