...
from Modules.mapping import rareloot
from Modules.display_object import DisplayObject

RARE_LOOT_COLOR = (0, 255, 255)
RARE_LOOT_LABEL_COLOR = (0, 255, 255, 255)
CIRCLE_SIZE = 3  # The size of the indicator circle we want


class RareLoot(DisplayObject):
    """
    Class to generate information for a rare loot object in memory
    """
    ...
    def __init__(self, memory_reader, actor_id, address, my_coords, raw_name):
        ...
        All of this data represents a "Rare Loot Item". If you want to add more, you will
        ...
        # Generate our rare loot's info
        self.name = rareloot.get(self.raw_name).get("Name")
    ...
    def _build_text_render(self) -> Label:
        ...
        :rtype: Label
        :return: What text we want displayed next to the rare loot
        """
        if self.screen_coords:
            return Label(self.text_str,
                         color=RARE_LOOT_LABEL_COLOR,
                         x=self.screen_coords[0] + TEXT_OFFSET_X,
                         y=self.screen_coords[1] + TEXT_OFFSET_Y,
                         batch=main_batch)

        return Label(self.text_str, color=RARE_LOOT_LABEL_COLOR, x=0, y=0, batch=main_batch)

    def update(self, my_coords: dict):
        """
        A generic method to update all the interesting data about a rare loot
