...
from Modules.mapping import genkey
from Modules.display_object import DisplayObject

GENERAL_KEY_COLOR = (255, 255, 255)
GENERAL_KEY_LABEL_COLOR = (255, 255, 255, 255)
CIRCLE_SIZE = 2  # The size of the indicator circle we want


class GeneralKey(DisplayObject):
    """
    Class to generate information for a general key object in memory
    """
    ...
    def __init__(self, memory_reader, actor_id, address, my_coords, raw_name):
        ...
        All of this data represents a "General Key". If you want to add more, you will
        ...
        # Generate our general key's info
        self.name = genkey.get(self.raw_name).get("Name")
    ...
    def _build_text_render(self) -> Label:
        ...
        :rtype: Label
        :return: What text we want displayed next to the general key
        """
        if self.screen_coords:
            return Label(self.text_str,
                         color=GENERAL_KEY_LABEL_COLOR,
        ...
        return Label(self.text_str, color=GENERAL_KEY_LABEL_COLOR, x=0, y=0, batch=main_batch)

    def update(self, my_coords: dict):
        """
        A generic method to update all the interesting data about a general key
