...
from Modules.mapping import megs
from Modules.display_object import DisplayObject

MEG_COLOR = (100, 51, 0)
MEG_LABEL_COLOR = (100, 51, 0, 255)
CIRCLE_SIZE = 5  # The size of the indicator circle we want


class Meg(DisplayObject):
    """
    Class to generate information for a meg object in memory
    """
    ...
    def __init__(self, memory_reader, actor_id, address, my_coords, raw_name):
        ...
        All of this data represents a "Meg". If you want to add more, you will
        ...
        # Generate our meg's info
        self.name = megs.get(self.raw_name).get("Name")
    ...
    def _built_text_string(self) -> str:
        """
        Generates a string used for rendering. Separate function in the meg
    ...
    def _build_text_render(self) -> Label:
        ...
        :rtype: Label
        :return: What text we want displayed next to the meg
        """
        if self.screen_coords:
            return Label(self.text_str,
                         color=MEG_LABEL_COLOR,
                         x=self.screen_coords[0] + TEXT_OFFSET_X,
                         y=self.screen_coords[1] + TEXT_OFFSET_Y,
                         batch=main_batch)

        return Label(self.text_str, color=MEG_LABEL_COLOR, x=0, y=0, batch=main_batch)

    def update(self, my_coords: dict):
        """
        A generic method to update all the interesting data about a meg
