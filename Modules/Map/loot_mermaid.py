...
LOOT_MERMAID_LABEL_COLOR = (255, 255, 255, 255)
CIRCLE_SIZE = 2  # The size of the indicator circle we want


class LootMermaid(DisplayObject):
    """
    Class to generate information for a loot mermaid object in memory
    """
    ...
    def __init__(self, memory_reader, actor_id, address, my_coords, raw_name):
        ...
        All of this data represents a "Loot Mermaid". If you want to add more, you will
        ...
        # Generate our loot mermaid's info
        self.name = lootmermaid.get(self.raw_name).get("Name")
    ...
    def _build_text_render(self) -> Label:
        ...
        :rtype: Label
        :return: What text we want displayed next to the loot mermaid
        """
        if self.screen_coords:
            return Label(self.text_str,
                         color=LOOT_MERMAID_LABEL_COLOR,
        ...
        return Label(self.text_str, color=LOOT_MERMAID_LABEL_COLOR, x=0, y=0, batch=main_batch)

    def update(self, my_coords: dict):
        """
        A generic method to update all the interesting data about a loot mermaid
